...
use App\Exports\BalanceGeneralFExport;
use Carbon\Carbon;
use DB;
use PDF;
...
class BalanceGeneralFController extends Controller
{
    public function proyectos(){
        $proyectos = DB::table('user_proyectos as a')
                            ...
                            ->pluck('nombre','id');
        return view('balance-general-f.proyectos',compact('proyectos'));
    }
    public function index($proyecto_id){
        ...
        return view('balance-general-f.index',compact('proyectos','proyecto_id','gestion'));
    }
    ...
    public function search(Request $request){
        ...
        return view('balance-general-f.show',compact('activos','pasivos','patrimonios','totales','cuentas','proyectos','proyecto_id','start_date','end_date','status_text','nroMaxColumna','total_activo','total_capital_pasivo'));
    }
    ...
    public function pdf(Request $request){
        ...
        $total_activo = $totales[$activos[0]->id];
        $total_capital_pasivo = $totales[$patrimonios[0]->id] + $totales[$pasivos[0]->id];
        $pdf = PDF::loadView('balance-general-f.pdf',compact('activos','pasivos','patrimonios','totales','cuentas','proyecto','start_date','end_date','status_text','nroMaxColumna','total_activo','total_capital_pasivo'));
    ...
    public function excel(Request $request){
        ...
        $total_activo = $totales[$activos[0]->id];
        $total_capital_pasivo = $totales[$patrimonios[0]->id] + $totales[$pasivos[0]->id];
        $file_name = 'BalanceGeneralF';
        return Excel::download(new BalanceGeneralFExport($activos,$pasivos,$patrimonios,$totales,$cuentas,$proyecto,$start_date,$end_date,$status_text,$nroMaxColumna,$total_activo,$total_capital_pasivo),$file_name . '.xlsx');
    }
